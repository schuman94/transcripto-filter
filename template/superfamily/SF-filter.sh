...
if [ -s $FASTA_ini/multiframe.fasta ]; then
    blastp -query $FASTA_ini/multiframe.fasta -db $DB -evalue 1e-6 -outfmt 10 -out $SIGNAL_OUT/multiframe.csv -num_threads 4
else
...
if [ -s $FASTA_post/multiframe_SF.fasta ]; then
    blastp -query $FASTA_post/multiframe_SF.fasta -db $DB2 -evalue 1e-6 -outfmt 10 -out $BLAST_OUT/multiframe_SF.csv -num_threads 4
else
...
if [ -s $FASTA_post/multiframe_NoSF.fasta ]; then
    blastp -query $FASTA_post/multiframe_NoSF.fasta -db $DB2 -evalue 1e-6 -outfmt 10 -out $BLAST_OUT/multiframe_NoSF.csv -num_threads 4
else
...
#NUEVO
python3 $P5 $BLAST_OUT/multiframe_SF.csv $SIGNAL_OUT/multiframe_SF.csv $DB2 $CSV_ini/multiframe.csv $RESULTADOS/multiframe_SF.csv
python3 $P5 $BLAST_OUT/multiframe_NoSF.csv $SIGNAL_OUT/multiframe_NoSF.csv $DB2 $CSV_ini/multiframe.csv $RESULTADOS/multiframe_NoSF.csv
...
python3 $P7 $BLAST_OUT/multiframe_NoSF.csv $CSV_ini/multiframe.csv $DB2 alineamientos_NoSF/multiframe/preMafft #NUEVO
